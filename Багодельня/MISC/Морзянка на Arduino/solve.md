## Overview
Светодиод передает флаг в формате азбуки Морзе. Нужно считать его послание – это флаг. Эмулятор работы ардуино с тестовым флагом: [https://wokwi.com/projects/441825314085368833](https://wokwi.com/projects/441825314085368833)

В прикрепленных файлах код передачи сигнала и видео-запись передачи настоящего флага.

---
Из morze.ino выбераем алфавит

Видео загружаем в любой редактор и замедляем его до 0.5

Что делает скрипт

Скрипт — это “моргание → скриншот → яркость → Морзе → текст”.
Основные этапы:
...
4. **Построение погов (точка/тире, пауза между буквами, пауза между словами).**
...
current_morse = ""
decoded_text = ""          # <--- буфер всей строки
...
def decode_current_morse():
    ...
    # дописываем в общий буфер
    decoded_text += ch

    # если поймали закрывающую фигурную скобку — вывести всё
    if ch == "}":
        print(f"\n[FULL] {decoded_text}")
        # можно очистить буфер, чтобы ловить следующую строку
        decoded_text = ""
